...
def ComputeResults(algo):
    for i, dataname in enumerate(datalist):
        true_df = pd.read_csv(truefilename.format(dataname, 'aracne'), index_col=0, sep='\t')
        f = dataname_dima.format(dataname, algo) if algo in algolist_dima else dataname_petr.format(dataname, algo) if algo in  algolist_petr else dataname_sergey.format(dataname, algo) 
        predicted_df = pd.read_csv(f, index_col=0, sep=',').T.abs() if algo in algolist_dima else pd.read_csv(f, index_col=0, sep='\t').abs() if algo in algolist_petr else pd.read_csv(f, sep=',').abs()
        
        predicted_df = predicted_df.reset_index()
        predicted_df.replace([np.inf, -np.inf], np.nan)
        predicted_df.fillna(0)
        # compute_aggregated_matrix(len(d_t) + len(p_t) + len(s_t), 
        ...
        if algo == '':
            ...
            algo1 = algo
        print("done", algo1, dataname, roc_auc)
        with open(saveresultsfile, "a") as f:
            f.write(algo1 + "\t" + dataname + "\t" + str(roc_auc) + "\n")


def ComputeAggregatedResults(algo):
    for i, dataname in enumerate(datalist):
        true_df = pd.read_csv(truefilename.format(dataname, 'aracne'), index_col=0, sep='\t')
        f = dataname_dima.format(dataname, algo) if algo in algolist_dima else dataname_petr.format(dataname, algo) if algo in  algolist_petr else dataname_sergey.format(dataname, algo) 
        
        frame = pd.read_csv(f, index_col=0, sep=',').T.abs() if algo in algolist_dima else pd.read_csv(f, index_col=0, sep='\t').abs() if algo in algolist_petr else pd.read_csv(f, sep=',').abs()
        predicted_df = compute_aggregated_matrix(frame)
                    ...
                    for s in range(2):
                        for s_t in combinations(algolist_sergey, s):
                            ...
                            for i, dataname in enumerate(datalist):
                                ...
                                print("done", algo, dataname, roc_auc)
                                with open(saveresultsfile, "a") as f:
                                    f.write(algo + "\t" + dataname + "\t" + str(roc_auc) + "\n")
